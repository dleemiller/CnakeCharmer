...
class RewardSystem:
    ...
    def register_scoring_function(self, 
    ...
    def calculate_reward(self, 
        ...
        if "execution" in outputs and not outputs["execution"].get("success", True):
            error_message = outputs["execution"].get("stderr", "Unknown execution error")
            error_score = -0.5  # Default penalty for execution failure
            logger.warning(f"Execution failed: {error_message[:200]}...")
            self.last_detailed_scores["error"] = f"Execution failed: {error_message[:200]}..."
            self.last_detailed_scores["total_score"] = error_score
            return error_score
        ...
        for func, weight, name in self.scoring_functions:
            try:
                ...
                logger.info(f"Scoring function '{name}' returned {score:.2f} (weighted: {weighted_score:.2f})")
        ...
        score_details = ", ".join([f"{name}={score:.2f} (weighted: {weighted_scores[name]:.2f})" 
    ...
    def get_score_explanation(self) -> str:
        ...
        for name, score in individual_scores.items():
            ...
            explanation.append(f"- {name}: {score:.2f} (weight: {weight:.2f}, weighted score: {weighted:.2f})")
...
def style_score(inputs: Dict[str, Any], outputs: Dict[str, Any]) -> float:
    ...
    logger.info(f"Style score details: line_length={line_length_score:.2f}, "
...
def documentation_score(inputs: Dict[str, Any], outputs: Dict[str, Any]) -> float:
    ...
    docstring_coverage = docstring_count / max(1, function_count) if function_count > 0 else 0
    ...
    logger.info(f"Documentation score details: module_docstring={0.2 if has_module_docstring else 0.0}, "
...
def cython_usage_score(inputs: Dict[str, Any], outputs: Dict[str, Any]) -> float:
    """
    Calculate a score based on Cython feature usage.
    ...
    is_cython = any(indicator in code for indicator in ["cdef", "cpdef", "cimport", "nogil", "# cython:"])
    if not is_cython:
    ...
    features = {
        ...
        'c_types': sum(1 for line in code_lines if any(t in line for t in 
                     ['int ', 'float ', 'double ', 'bint ', 'unsigned '])),
    ...
    logger.info(f"Cython usage score details: density_score={density_score:.2f} ({feature_density:.3f}), " 
...
def create_default_reward_system() -> RewardSystem:
    ...
    Returns:
        RewardSystem: Configured reward system
    """
    reward_system = RewardSystem()
    
    # Register basic scoring functions
    reward_system.register_scoring_function(
        style_score, weight=0.2, name="style"
    )
    ...
    reward_system.register_scoring_function(
        cython_usage_score, weight=0.5, name="cython_usage"
    )

...
class RefinableCodeGenerator(Module):
    ...
    def forward(self, prompt, hint_=None, **kwargs):
        ...
        try:
            ...
            if result.get("error"):
                logger.warning(
                    f"Request {request_id}: Code generation produced error: {result['error'][:100]}..."
                )
            else:
...
def code_quality_reward(inputs, outputs):
    ...
    cython_expected = any(
        keyword in prompt
        for keyword in ["cython", "fast", "performance", "efficient", "speed", "optimize"]
    )

    # Detect actual Cython usage levels
    is_cython_file = any(
        indicator in code for indicator in ["# cython:", "cimport", "cpdef", "cdef class"]
    )
    ...
    cython_features = {
        ...
        "typed_args": sum(
            ...
            if ":" in line and any(t in line for t in ["int", "float", "double", "long", "bint"])
        ),
        "c_types": sum(
            1
            for line in code_lines
            if any(t in line for t in ["int ", "float ", "double ", "bint ", "unsigned "])
        ),
    ...
    if cython_expected:
        ...
        if is_cython_file:
            ...
            feature_density = total_cython_features / max(
                1, (total_lines - cython_features["directives"])
            )
            ...
            if (
                not has_py_objects
                and cython_features["c_types"] > 0
                and cython_features["nogil"] > 0
            ):
                cython_score += 0.3
    ...
    for _i, line in enumerate(code_lines[:10]):  # Check first 10 lines
    ...
    max_total = (
        max_scores["base"]
        + (max_scores["cython"] * weights["cython"])
        + (max_scores["docs"] * weights["docs"])
        + (max_scores["style"] * weights["style"])
    ...
    if is_cython_file:
        ...
        logger.debug(
            f"Cython feature density: {total_cython_features / max(1, total_lines):.2f} features per line"
        )
...
def create_refinable_code_generator(max_iters=1, refine_iterations=3, threshold=1.0):
    ...
    refinable_code_gen = dspy.Refine(
        module=generator, N=refine_iterations, reward_fn=code_quality_reward, threshold=threshold
    )
...
if __name__ == "__main__":
    ...
    lm = dspy.LM(
        model="openrouter/anthropic/claude-3.7-sonnet:thinking", cache=False, max_tokens=16000
    )

"""Policy-tree based finite-horizon value iteration skeleton."""
        ...
        for s in states:
            ...
            for sp in states:
                for o in observations:
                    ...
                    subtree_value = self.children[o].values[s] if self.children else 0.0
...
class ValueIteration:
    def __init__(self, horizon, discount_factor=0.9):
        if not (isinstance(horizon, int) and horizon >= 1):
            raise ValueError("Horizon must be integer >=1")
        self._discount_factor = discount_factor

...
"""Elementwise mul f32 kernels — scalar and AVX2 implementations.
...
cdef void elementwise_mul_f32(const float *a, const float *b,
...
cdef void elementwise_mul_f32_avx(const float *a, const float *b,
                                  float *out, int n) noexcept nogil:
    """AVX2 elementwise mul: 16 floats per iteration, 8-wide remainder."""
    ...
    for i in range(0, end16, 16):
        ...
        _mm256_storeu_ps(&out[i], _mm256_mul_ps(va0, vb0))
        _mm256_storeu_ps(&out[i + 8], _mm256_mul_ps(va1, vb1))
